...
class DocumentProcessor:
    ...
    def process_pdf(self, file_path):
        """Process PDF file and detect PII"""
        ...
        for page_num in range(len(doc)):
            ...
            # Detect PII
            pii_items = self.pii_detector.detect_pii(text)
            
            # Get text blocks with positions for more precise redaction
...
def redact_document():
    ...
    try:
        ...
        if document_type == 'pdf':
            result_path = processor.redact_pdf(original_path, redactions, output_path)
